...
let mixin = {
  ...
  methods: {
    specialInit: function () {
      let self = this
      BUS.on(BUS.EVENTS.TENANT, self.handleDateRangeChange)
    },
    ...
    handleSpecialMetric (metricObj, queryStr) {
      // 业务系统繁忙度没有type字段，手工增加该字段，用于展示总体繁忙度
      let name = metricObj && metricObj.__name__
      if(typeof(name) == 'undefined' && typeof(queryStr) != 'undefined'){
        if(queryStr.indexOf("avg(te_score{") ==0){
 ...
 */
    handleRangeResponse (list, queryStr) {
      let obj = {}
      let info = null
      for (let i in list) {
        let item = list[i]
        if (this.handleSpecialMetric) {
          this.handleSpecialMetric(item.metric, queryStr)
        }
    ...
    getMetric (cb) {
      ...
      let count = 0
      let resList = []
      let done = function (queryStr) {
        let obj = self.handleRangeResponse(resList, queryStr)
        cb && cb(obj)
      }
