<template>
  <div class="rank-container main-trend">
    <div class="lc2-header">
      <span>{{title}}趋势</span>
      <span v-tooltip='getTip("rp.busy")' class="info-tooltip"></span>
    </div>
...
import MixinLine from './Mixin_Rp_Line'
...
export default {
  ...
  data () {
    let obj = {
      title: '繁忙度',
      chartId: Metric.name.rp.resource_busy,
      type: 'rp',
      chartConfig: {
        title: '繁忙度',
        yTitle: '繁忙度',
        legends: ['裸金属', '云主机', 'KVM宿主机']
      },
      dateRangeConfig: null,
      itemId: this.$route.query.id, // get from router
      chartOpt: null,
      metrics: [Metric.name.rp.resource_busy]
    }
    return obj
  }
}
